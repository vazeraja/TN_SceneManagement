    ...
    class DragAndDropManipulator : PointerManipulator {
        ...
        void OnPointerDown(PointerDownEvent _) {
            ...
            if (droppedObject != null) {
                ...
                UnityEditor.DragAndDrop.objectReferences = new[] {droppedObject};
                if (assetPath != string.Empty) {
                    UnityEditor.DragAndDrop.paths = new[] {assetPath};
                }
                else {
        ...
        void OnDragEnter(DragEnterEvent _) {
            ...
            if (UnityEditor.DragAndDrop.paths.Length > 0) {
                assetPath = UnityEditor.DragAndDrop.paths[0];
                var splitPath = assetPath.Split('/');
                draggedName = splitPath[splitPath.Length - 1];
            }
            else if (UnityEditor.DragAndDrop.objectReferences.Length > 0) {
                draggedName = UnityEditor.DragAndDrop.objectReferences[0].name;
            }
        ...
        void OnDragPerform(DragPerformEvent _) {
            ...
            droppedObject = UnityEditor.DragAndDrop.objectReferences[0];
            string draggedName;
            if (assetPath != string.Empty) {
                var splitPath = assetPath.Split('/');
                draggedName = splitPath[splitPath.Length - 1];
